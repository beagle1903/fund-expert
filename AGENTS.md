...
bundle.resolve_active_bundle (versioned current.json or validated legacy files)
  ...
  → scoring.score.score_candidates (weighted sum of R̂, V̂, 1−F̂, M̂; minus SRRI risk penalty; keeps per-signal contribution columns)
  ...
  → (--news only) news.penalty.apply_negative_news_penalty (top-K Tavily query, −0.20 binary penalty per fund with hits; records `news_penalty`)
